...
class cmb_Meta_Box_ajax {
	...
	public static function get_oembed( $url, $object_id, $args = array() ) {
		...
		// ping Solo SM for an embed
		$check_embed = $wp_embed->run_shortcode( '[embed'. $embed_args .']'. $oembed_url .'[/embed]' );

		// fallback that Solo SM creates when no oEmbed was found
		...
		return '<p class="ui-state-error-text">'. sprintf( __( 'No oEmbed Results Found for %s. View more info at', 'cmb' ), $fallback ) .' <a href="http://codex.Solo SM.org/Embeds" target="_blank">codex.Solo SM.org/Embeds</a>.</p>';
